...
[Setup]
AppId={{A1B2C3D4-ARCA-KOR1-8000-PATCHINSTALL}
AppName={#MyAppName}
...
OutputBaseFilename=Arcanum_Korean_Patch_v1.8
OutputDir=output
Compression=lzma2
...
[Messages]
ko.WelcomeLabel2=아카넘 한글 패치 v{#MyAppVersion}%n%nUAP와 고해상도 패치가 이미 설치된 유저용 업데이트입니다.%n%n※ v1.8 변경사항:%n  - NPC 이름 표시 버그 수정 (블랙루트 도둑 캠프 등 13명)%n  - Vormantown 모듈 자동 제거 (충돌 방지)%n%n신규 설치는 올인원 패치를 사용하세요.%n%n계속하려면 [다음]을 클릭하세요.
